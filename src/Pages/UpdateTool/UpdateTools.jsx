...
const UpdateTool = () => {
  ...
  useEffect(() => {
    if (Object.keys(data).length > 0) {
      setToolName(data?.category?.name);
      setPrice(data?.category?.price);
      setDescription(data?.category?.description);
      setCurrentImage(data?.category?.image);
      setCategory(data?.category.category);
      setSelectedBrand(data?.category?.brand);
      setFunctionalities(data?.category?.functionalities);
      setKits(data?.category?.kits);
    }
  }, [data]);
  ...
  const uploadTools = async (e) => {
    ...
    try {
      ...
      setIsLoading(true);
      await axios.put(toolsUrl + `tool/${id}`, formData);
      toast.success("Tool udpated succesfully");
      navigate("/");
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
  ...
  const deleteTool = async () => {
    setIsLoading(true);
    try {
      await axios.delete(toolsUrl + `tool/${id}`);
      setIsLoading(false);
      toast.success("Tools Delete succesfully");
      navigate("/");
    } catch (error) {
      setIsLoading(false);
      console.log(error);
  ...
  return (
    <div className="pt-[80px] pb-10 px-4 bg-gradient-to-r to-[#611055] from-[#104212] text-white min-h-[100vh]">
      {toolisLoading ? (
        ...
        <div className="max-w-[650px] mx-auto">
          <div className="flex justify-center mb-3">
            <h4 className="text-xl font-semibold tracking-wide">Update Tool</h4>
          </div>
          <div>
