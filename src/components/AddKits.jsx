...
const AddKitsModal = ({ isOpen, onOpenChange }) => {
  ...
  const addKit = async () => {
    if (kit.length <= 0 || description.length <= 0 || !selectedFile) {
    ...
    try {
      setIsLoading(true);
      const response = await axios.post(`${toolsUrl}kits/`, formData);
      setIsLoading(false);
      setKit("");
      setDescription("");
